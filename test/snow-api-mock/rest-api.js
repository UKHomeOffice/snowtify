...
const validFuture = str => (date => date.isValid() && date.isAfter(moment()))(moment(str, 'YYYY-MM-DD HH:mm:ss'));
...
module.exports = {
  ...
  template: (params, query, body, cookies, headers) => {
    ...
    if (body.messageid === 'HO_SIAM_IN_REST_CHG_POST_JSON') {
      ...
      expect(json).to.be.an('object').that.includes.keys(['title', 'endTime', 'description', 'supplierRef']);
      response = {
        result: {
      ...
      response = {
        result: {
          status: !body.internal_identifier || !body.external_identifier ||
            body.internal_identifier.startsWith('!') || body.external_identifier.startsWith('!') ? '7' : '5'
        }
      };
    }
